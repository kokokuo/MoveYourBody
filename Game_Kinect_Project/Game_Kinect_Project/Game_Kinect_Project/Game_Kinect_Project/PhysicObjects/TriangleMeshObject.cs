...
namespace Game_Kinect_Project.PhysicObjects
{
    class TriangleMeshObject : PhysicObject
    {
        ...
        public void SetApplyLocalTransform(Vector3 pos) 
        {
            JigLibX.Math.Transform s = new JigLibX.Math.Transform(new Vector3(0, 0, 0), Matrix.Identity);
            
            JigLibX.Math.Transform d=s;
            d.Orientation = Matrix.Identity;
            d.Position = new Vector3(0, 0, 0);
            //d.Position=pos;
            collision.ApplyLocalTransform(new JigLibX.Math.Transform(pos, Matrix.Identity));
        }
